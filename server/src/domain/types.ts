 ...
 *                       floor when the MIS feed is failing (so a symbol shows an
 *                       official number instead of nothing). Absent ⇒ "mis".
 */
...
/** Live-feed health snapshot, surfaced by `/api/health`. */
...
/** Overall system health, surfaced by `/api/health`. */
